...
本番リリース前の検証環境です。prod と同等のスペック・セキュリティ構成で、冗長性のみコスト削減しています。

| 項目 | 設定 |
|---|---|
| **デプロイトリガー** | `workflow_dispatch` による手動実行 |
...
| リソース | スペック |
|---|---|
| ECS Fargate | 1 vCPU / 2048 MB / 1 タスク / Auto Scaling なし |
| RDS | db.t3.medium / 30 GB / Single-AZ / バックアップ 14 日 / 削除保護あり |
| NAT Gateway | 1（シングル AZ） |
| WAF | あり |
| ログ保持 | 30 日 |
...
| **ECS CPU** | 256 | 1024 | 1024 |
| **ECS メモリ** | 512 MB | 2048 MB | 2048 MB |
| **ECS タスク数** | 1 | 1 | 2-4 |
| **Auto Scaling** | なし | なし | CPU 70% ターゲット |
| **RDS インスタンス** | db.t3.small | db.t3.medium | db.r6g.large |
| **RDS ストレージ** | 20 GB | 30 GB | 50 GB |
| **Multi-AZ** | No | No | Yes |
